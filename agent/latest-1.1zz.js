...
`;
    ...
    if (url.pathname === "/api/report" && request.method === "POST") {
      try {
        ...
        if (data.logs) {
          // D1 写保护：OpenVPN 每小时 TLS 软重置/握手会产生大量重复日志(如 TLS soft reset / VERIFY OK / Control Channel)
          // 落库前按行过滤噪音，只保留业务日志行，显著降低 D1 写入量与存储膨胀
          const cleanLogs = String(data.logs || "").split("\n").filter(line => {
            const l = line.trim();
            if (!l) return false;
            if (/TLS:\s*(soft\s*reset|tls_process)/.test(l)) return false;
            if (/VERIFY\s*OK:/i.test(l)) return false;
            if (/Control\s*Channel:/i.test(l)) return false;
            if (/TLSv1\.[23],?\s+cipher/.test(l)) return false;
            if (/killed\s*expiring\s*key/.test(l)) return false;
            return true;
          }).join("\n").trim().slice(-16000);
          if (cleanLogs) {
