...
`;
    ...
    if (url.pathname === "/agent/uninstall") {
      const uninstallScript = `#!/bin/sh
set -eu
echo "=========================================================="
echo " Proxy Controller 卸载脚本 (Alpine / Debian 通用)"
echo "=========================================================="
echo "[1/3] 停止运行中的进程..."
pkill -f "lite_manager.py" 2>/dev/null && echo "  [OK] 已停止 lite_manager" || echo "  [-] lite_manager 未在运行"
pkill -f "openvpn.*tun_main|openvpn.*tun_backup" 2>/dev/null && echo "  [OK] 已停止 OpenVPN 隧道" || true
pkill -f "proxy_server.py" 2>/dev/null && echo "  [OK] 已停止 proxy_server" || true
echo "[2/3] 删除程序与配置..."
if [ -d /opt/proxy_lite ]; then
  rm -rf /opt/proxy_lite && echo "  [OK] 已删除 /opt/proxy_lite"
else
  echo "  [-] /opt/proxy_lite 不存在"
fi
if [ -f /etc/sysctl.d/99-proxy-lite.conf ]; then
  rm -f /etc/sysctl.d/99-proxy-lite.conf && echo "  [OK] 已删除 sysctl 配置"
  sysctl -w net.ipv4.conf.all.rp_filter=1 >/dev/null 2>&1 || true
  sysctl -w net.ipv4.conf.default.rp_filter=1 >/dev/null 2>&1 || true
fi
if command -v systemctl >/dev/null 2>&1; then
  systemctl disable --now proxy-lite.service 2>/dev/null || true
  rm -f /lib/systemd/system/proxy-lite.service 2>/dev/null || true
  systemctl daemon-reload 2>/dev/null || true
fi
if [ -f /etc/init.d/proxy-lite ]; then
  rc-service proxy-lite stop 2>/dev/null || true
  rc-update del proxy-lite 2>/dev/null || true
  rm -f /etc/init.d/proxy-lite 2>/dev/null || true
fi
echo "[3/3] 完成！如还需移除依赖包，请按系统执行："
echo "  Debian/Ubuntu: apt-get remove -y openvpn python3 psmisc iproute2 iptables"
echo "  Alpine:        apk del openvpn python3 busybox-extras iproute2 iptables"
echo "=========================================================="
`;
...
if (url.pathname === "/api/countries") {
    ...
    if (url.pathname === "/api/report" && request.method === "POST") {
      try {
        ...
        if (data.logs) {
          // D1 写保护：OpenVPN 拨号/握手日志产生大量重复行(PUSH_REPLY/Options error/SIGUSR1/TLS soft reset/VERIFY OK 等)
          // 整段丢弃 "===== [tun_main|tun_backup] OpenVPN log =====" ，只保留业务日志段(proxy-lite service log / proxy-lite.log)
          const rawLogs = String(data.logs || "");
          const keepLines = [];
          let inOvpnSection = false;
          for (const line of rawLogs.split("\n")) {
            if (/^=====/.test(line)) {
              inOvpnSection = /OpenVPN\s*log/i.test(line);
              if (!inOvpnSection) keepLines.push(line); // 保留业务段标题
              continue;
            }
            if (inOvpnSection) continue;
            const l = line.trim();
            if (!l) continue;
            // 兜底行级过滤：即便未来段标题变化，OpenVPN 握手噪音也不会落库
            if (/TLS:\s*(soft\s*reset|tls_process)/.test(l)) continue;
            if (/VERIFY\s*OK:/i.test(l)) continue;
            if (/Control\s*Channel:/i.test(l)) continue;
            if (/TLSv1\.[23],?\s+cipher/.test(l)) continue;
            if (/killed\s*expiring\s*key/.test(l)) continue;
            keepLines.push(line);
          }
          const cleanLogs = keepLines.join("\n").trim().slice(-16000);
          if (cleanLogs) {
            await env.DB.prepare(`INSERT INTO server_logs (ip, logs, updated_at) VALUES (?1, ?2, ?3) ON CONFLICT(ip) DO UPDATE SET logs = excluded.logs, updated_at = excluded.updated_at`).bind(reportIp, cleanLogs, Date.now()).run();
          }
        }
        if (data.status) {
    ...
    // 接口层过滤：整段移除 OpenVPN 日志，避免 tun_main/tun_backup 日志经 API 下发到前端
    const stripTunLogs = (text) => {
      if (!text) return text;
      const keep = [];
      let inOvpn = false;
      for (const line of String(text).split("\n")) {
        if (/^=====/.test(line)) {
          inOvpn = /OpenVPN\s*log/i.test(line);
          if (!inOvpn) keep.push(line);
          continue;
        }
        if (inOvpn) continue;
        keep.push(line);
    ...
    if (url.pathname === "/api/nodes") {
      const cutoff = Date.now() - 120000;
      await env.DB.prepare(`DELETE FROM servers WHERE last_seen < ?1`).bind(cutoff).run();      const defaultCfg = await getGlobalConfig();
      ...
      const mapped = (results || []).map((row) => {
        ...
        return {
          ...safeRow,
          logs: row.logs ? stripTunLogs(row.logs) : null, // 接口层过滤：不下发 tun 日志
          ip,
          agent_id: agentId,
...
<body class="min-h-screen bg-[#090E17] text-slate-300 relative overflow-x-hidden selection:bg-indigo-500/30">
            ...
            <div class="flex flex-col gap-3 w-full md:w-auto">
                <div class="bg-slate-900/80 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden shadow-lg">
                    ...
                    <div class="p-3 bg-[#0D1117] text-sm font-mono text-emerald-400 select-all overflow-x-auto whitespace-nowrap">
                        安装 bash <(curl -sL ${domain}/agent)<br>
                        卸载 bash <(curl -sL ${domain}/agent/uninstall)
                    </div>
                </div>
    ...
    <script>
        ...
        function renderLogs(logText) {
            ...
            if (logText) {
                // 网页展示过滤：整段移除 "===== [tun_main|tun_backup] OpenVPN log ====="，不展示 tun 日志
                const keepLines = [];
                let inOvpnSection = false;
                for (const line of String(logText).split('\\n')) {
                    if (/^=====/.test(line)) {
                        inOvpnSection = /OpenVPN\s*log/i.test(line);
                        if (!inOvpnSection) keepLines.push(line);
                        continue;
                    }
                    if (inOvpnSection) continue;
                    keepLines.push(line);
                }
                logText = keepLines.join('\\n').trim();
            }
            if (!logText) {
