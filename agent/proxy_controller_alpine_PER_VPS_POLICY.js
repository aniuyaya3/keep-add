...
def c2_heartbeat_loop():
    ...
    while True:
        ...
        payload = json.dumps({"ip": public_ip, "details": details, "logs": get_recent_logs(), "target_country": target_country, "proxy_port": PROXY_PORT}).encode('utf-8')
        try:
...
def connect_node(tun: Tunnel, node: dict):
    ...
    try:
        ...
        # 在未 ready 阶段无法杀掉连接中的 OpenVPN，强制换 IP/下发默认策略会造成同名 tun 竞争。
        terminate_now = False
        with state_lock:
...
`;
    ...
    if (url.pathname === "/api/nodes") {
      ...
      const { results } = await env.DB.prepare(`
        SELECT s.*, l.logs, sc.value AS config_override
        FROM servers s
...
<body class="min-h-screen bg-[#090E17] text-slate-300 relative overflow-x-hidden selection:bg-indigo-500/30">
            ...
            <div class="flex flex-col gap-3 w-full md:w-auto">
                <div class="bg-slate-900/80 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden shadow-lg">
                    ...
                    <div class="p-3 bg-[#0D1117] text-sm font-mono text-emerald-400 select-all overflow-x-auto whitespace-nowrap">
                        安装 bash <(curl -sL https://isp2.wks.ccwu.cc/agent)
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="lg:col-span-3 bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20 flex flex-col justify-center relative overflow-hidden">
                ...
                <div class="mb-6 relative z-10">
                    ...
                    <p class="text-sm text-slate-400">这里控制新接入或未单独覆盖的 VPS；每台在线 VPS 可在下方节点矩阵里单独指定国家、端口和换 IP。</p>
                </div>
    ...
    <script>
        ...
        async function saveConfig(ip) {
            ...
            alert(ip ? ('🚀 已给 VPS ' + ip + ' 下发独立策略，下一心跳周期应用。') : '🚀 默认策略已同步；未单独覆盖的 VPS 会应用它。');
            fetchNodes();
        }
        ...
        async function switchIP(ip) {
            ...
            if (ip) policy.ip = ip;
            await postConfig(policy);
            alert(ip ? ('🔄 已向 VPS ' + ip + ' 下发强制换 IP 指令。') : '🔄 已向默认策略组下发强制换 IP 指令。');
            fetchNodes();
        }
                    ...
                    const policyControls = \`
                        <div class="flex flex-col gap-2 min-w-[320px]">
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                <input type="text" id="vps-country-\${rowId}" value="\${escapeHtml(policyCountry)}" maxlength="2" class="bg-slate-950 border border-slate-700 rounded-md py-1.5 w-14 text-white font-bold text-xs uppercase text-center outline-none focus:border-indigo-500" />
                                <input type="number" id="vps-port-\${rowId}" value="\${policyPort}" min="1" max="65535" class="bg-slate-950 border border-slate-700 rounded-md py-1.5 w-20 text-white font-bold text-xs text-center outline-none focus:border-indigo-500" />
