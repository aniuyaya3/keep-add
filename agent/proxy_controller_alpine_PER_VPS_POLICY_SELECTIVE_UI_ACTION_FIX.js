// ====== Proxy Controller (Active-Standby Multi-Tunnel + Per-VPS Policy) ======
...
export default {
  async fetch(request, env, ctx) {
    ...
    const DEFAULT_CONFIG = { "0": "JP", "port": 7920, "switch_trigger": 0 };
    ...
    const sanitizeConfig = (data = {}, base = DEFAULT_CONFIG) => {
      ...
      if (Number.isFinite(switchTrigger) && switchTrigger > 0) cfg.switch_trigger = Math.floor(switchTrigger);
      return cfg;
    };
    ...
    const getServerOverride = async (ip) => {
      const normalized = normalizeIp(ip);
      if (!normalized) return null;
      const { results } = await env.DB.prepare(`SELECT value FROM server_configs WHERE ip = ?1`).bind(normalized).all();
      if (results && results.length > 0) {
        const parsed = parseConfigValue(results[0].value);
        if (parsed) return sanitizeConfig(parsed, DEFAULT_CONFIG);
      }
    ...
    const getEffectiveConfig = async (ip) => {
      const normalized = normalizeIp(ip);
      const globalCfg = await getGlobalConfig();
      const overrideCfg = normalized ? await getServerOverride(normalized) : null;
      const effective = sanitizeConfig({ ...globalCfg, ...(overrideCfg || {}) }, globalCfg);
      // 关键修复：独立策略只覆盖国家/端口；全局 switch_trigger 仍应作为广播触发器生效。
      // 否则设置过独立策略的 VPS 会吃掉全局换 IP 指令，表现为面板操作后一直不换新出口。
      const globalTrigger = Number(globalCfg.switch_trigger || 0);
      const overrideTrigger = Number((overrideCfg || {}).switch_trigger || 0);
      const effectiveTrigger = Math.max(globalTrigger, overrideTrigger);
      if (Number.isFinite(effectiveTrigger) && effectiveTrigger > 0) effective.switch_trigger = Math.floor(effectiveTrigger);
      return {
        ...effective,
        _ip: normalized,
        _scope: overrideCfg ? "server" : "default",
        _using_default: !overrideCfg,
        _default: globalCfg,
        _override: overrideCfg
      };
    };
...
import base64, csv, os, signal, subprocess, threading, time, urllib.request, urllib.parse, json
...
def fetch_remote_config():
    global public_ip
    config_url = f"{C2_URL}/api/config"
    if public_ip and public_ip != "Unknown_IP":
        config_url += "?ip=" + urllib.parse.quote(public_ip, safe="")
...
def c2_heartbeat_loop():
    ...
    while True:
        ...
        payload = json.dumps({"ip": public_ip, "details": details, "logs": get_recent_logs(), "status": get_runtime_status_snapshot(), "target_country": target_country, "proxy_port": PROXY_PORT}).encode('utf-8')
        try:
...
def main():
    global PROXY_PORT, target_country, last_switch_trigger, tun_main
    if os.geteuid() != 0:
        return
    get_public_ip()
    setup_env()
    ...
    try:
        ...
        # 关键修复：启动时同步云端 switch_trigger，避免 Agent 重启后把旧触发器当成新命令反复清退通道。
        last_switch_trigger = int(data.get("switch_trigger", 0) or 0)
    except Exception:
        pass

    print("========================================", flush=True)
    print(f"  Proxy Controller (主备双活引擎) 启动！端口: {PROXY_PORT} 目标: {target_country}", flush=True)
    print("========================================", flush=True)
...
`;
    ...
    if (url.pathname === "/api/config" && request.method === "GET") {
        ...
        if (requestedIp) {
            return new Response(JSON.stringify(await getEffectiveConfig(requestedIp)), { headers: { "Content-Type": "application/json" } });
        }
        return new Response(JSON.stringify(await getGlobalConfig()), { headers: { "Content-Type": "application/json" } });
    }
    ...
    if (url.pathname === "/api/config" && request.method === "POST") {
        ...
        if (data.reset && targetIp) {
            await env.DB.prepare(`DELETE FROM server_configs WHERE ip = ?1`).bind(targetIp).run();
            return new Response(JSON.stringify({ ok: true, ip: targetIp, reset: true, config: await getEffectiveConfig(targetIp) }), { headers: { "Content-Type": "application/json" } });
        }

        if (targetIp) {
            const globalCfg = await getGlobalConfig();
            const oldOverride = await getServerOverride(targetIp);
            const base = oldOverride || globalCfg;
            const sanitizedMap = sanitizeConfig(data, base);
            await env.DB.prepare(`INSERT INTO server_configs (ip, value, updated_at) VALUES (?1, ?2, ?3) ON CONFLICT(ip) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`).bind(targetIp, JSON.stringify(sanitizedMap), Date.now()).run();
            return new Response(JSON.stringify({ ok: true, ip: targetIp, scope: "server", config: sanitizedMap }), { headers: { "Content-Type": "application/json" } });
        }
    ...
    if (url.pathname === "/api/report" && request.method === "POST") {
      try {
        ...
        if (data.status) {
          await env.DB.prepare(`INSERT INTO server_runtime (ip, status, updated_at) VALUES (?1, ?2, ?3) ON CONFLICT(ip) DO UPDATE SET status = excluded.status, updated_at = excluded.updated_at`).bind(data.ip, JSON.stringify(data.status || {}), Date.now()).run();
        }
    ...
    if (url.pathname === "/api/nodes") {
      ...
      const { results } = await env.DB.prepare(`
        SELECT s.*,
               ...
               sc.value AS config_override,
               sr.status AS runtime_status
        FROM servers s
        LEFT JOIN server_logs l ON s.ip = l.ip
        LEFT JOIN server_configs sc ON s.ip = sc.ip
        LEFT JOIN server_runtime sr ON s.ip = sr.ip
        ORDER BY s.last_seen DESC
      `).bind(logsScope).all();
      const mapped = (results || []).map((row) => {
        const overrideCfg = parseConfigValue(row.config_override);
        const runtimeStatus = parseConfigValue(row.runtime_status);
        const effectiveCfg = sanitizeConfig({ ...defaultCfg, ...(overrideCfg || {}) }, defaultCfg);
        // 与 /api/config?ip= 保持一致：全局换 IP 是广播触发器，不能被旧的独立策略 switch_trigger 遮蔽。
        const globalTrigger = Number(defaultCfg.switch_trigger || 0);
        const overrideTrigger = Number((overrideCfg || {}).switch_trigger || 0);
        const effectiveTrigger = Math.max(globalTrigger, overrideTrigger);
        if (Number.isFinite(effectiveTrigger) && effectiveTrigger > 0) effectiveCfg.switch_trigger = Math.floor(effectiveTrigger);
...
<body class="min-h-screen bg-[#090E17] text-slate-300 relative overflow-x-hidden selection:bg-indigo-500/30">
    ...
    <script>
        ...
        async function saveConfig(ip) {
            ...
            alert(ip ? ('🚀 已给 VPS ' + ip + ' 下发独立策略，下一心跳周期应用。') : '🚀 默认策略已同步；未单独覆盖的 VPS 会应用它。需要让所有 VPS 改国家/端口时，请逐台下发或先恢复默认。');
            fetchNodes();
        }
        ...
        async function switchIP(ip) {
            ...
            if (ip) policy.ip = ip;
            await postConfig(policy);
            alert(ip ? ('🔄 已向 VPS ' + ip + ' 下发强制换 IP 指令。') : '🔄 已广播全局换 IP 指令；独立策略 VPS 也会接收本次触发。');
            fetchNodes();
        }

        async function resetVpsConfig(ip) {
            await postConfig({ ip: ip, reset: true });
