...
def get_recent_logs():
    # Alpine 兼容：无 systemd journal，改为读取独立运行日志文件末尾 30 行
    try:
...
echo "    Alpine Edition | Standalone Trigger | Viewable     "
echo "=========================================================="
set -e

# 检查是否 Alpine（Alpine 使用 apk，无 systemd）
if command -v apk >/dev/null 2>&1; then
    PKG_MGR="apk"
elif command -v apt-get >/dev/null 2>&1; then
    echo "[!] 检测到 Debian/Ubuntu。当前为 Alpine 兼容版，请使用 Alpine Linux(apk)。"
    exit 1
else
    echo "[!] 无法识别的包管理器，仅支持 Alpine Linux。"
    exit 1
fi
...
apk update -q
apk add --no-cache openvpn python3 curl iproute2 iptables busybox-extras psmisc
...
echo "[2/3] 生成独立触发管理脚本(无 systemd)..."
# ---- start.sh: 独立启动（后台运行，日志写入 proxy_lite.log）----
cat > /opt/proxy_lite/start.sh << 'EOF'
#!/usr/bin/env bash
cd /opt/proxy_lite
pkill -f "lite_manager.py" 2>/dev/null || true
sleep 1
nohup /usr/bin/python3 -u lite_manager.py >> /opt/proxy_lite/proxy_lite.log 2>&1 &
...
chmod +x /opt/proxy_lite/start.sh /opt/proxy_lite/stop.sh /opt/proxy_lite/status.sh /opt/proxy_lite/logs.sh

echo "[3/3] 启动引擎(独立触发)..."
/opt/proxy_lite/start.sh

echo "=========================================================="
echo "  [+] 部署完成！(Alpine 独立触发版)  "
...
echo "  日志文件: /opt/proxy_lite/proxy_lite.log  "
echo "=========================================================="
`;
