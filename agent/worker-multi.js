...
def get_recent_logs():
    # 多系统兼容：无 systemd journal(如 Alpine)时读取独立运行日志文件末尾 30 行
    log_path = WORKSPACE / "proxy_lite.log"
    if log_path.exists():
        try:
...
# ==========================================================
#   Proxy Controller (Active-Standby Multi-Tunnel)
#   Auto-Detect OS | Standalone Trigger | Viewable | Uninstall
# ==========================================================
set -e
...
detect_os() {
    if command -v apk >/dev/null 2>&1; then
        OS="Alpine"; PM_CMD="apk add --no-cache"; PM_UPDATE="apk update -q"
        DEPS="openvpn python3 curl iproute2 iptables busybox-extras"
    elif command -v apt-get >/dev/null 2>&1; then
        OS="Debian/Ubuntu"; PM_CMD="apt-get install -y"; PM_UPDATE="apt-get update -q"
        DEPS="openvpn python3 curl iproute2 iptables psmisc"
    elif command -v dnf >/dev/null 2>&1; then
        OS="RHEL/Fedora"; PM_CMD="dnf install -y"; PM_UPDATE="true"
        DEPS="openvpn python3 curl iproute iptables procps-ng"
    elif command -v yum >/dev/null 2>&1; then
        OS="RHEL/CentOS"; PM_CMD="yum install -y"; PM_UPDATE="true"
        DEPS="openvpn python3 curl iproute iptables procps-ng"
    elif command -v pacman >/dev/null 2>&1; then
        OS="Arch"; PM_CMD="pacman -S --noconfirm --needed"; PM_UPDATE="true"
        DEPS="openvpn python3 curl iproute2 iptables procps-ng"
    else
        echo "[!] 无法自动识别的操作系统/包管理器"
        echo "    请手动安装: openvpn python3 curl iproute2 iptables <psmisc|procps-ng>"
        exit 1
    fi
}
detect_os
...
# start.sh: 独立启动
cat > /opt/proxy_lite/start.sh << 'EOF'
#!/usr/bin/env bash
cd /opt/proxy_lite
pkill -f "lite_manager.py" 2>/dev/null || true
sleep 1
PYBIN="$(command -v python3 || echo /usr/bin/python3)"
nohup "$PYBIN" -u lite_manager.py >> /opt/proxy_lite/proxy_lite.log 2>&1 &
echo "[+] 已后台启动 lite_manager (PID: $!)"
echo "[+] 日志: /opt/proxy_lite/proxy_lite.log"
EOF

# stop.sh: 独立停止
cat > /opt/proxy_lite/stop.sh << 'EOF'
#!/usr/bin/env bash
pkill -f "lite_manager.py" 2>/dev/null && echo "[-] 已停止 lite_manager" || echo "[-] lite_manager 未在运行"
EOF

# status.sh: 查看状态 + 最近日志
cat > /opt/proxy_lite/status.sh << 'EOF'
#!/usr/bin/env bash
echo "=== Proxy Controller 状态 ==="
if pgrep -f "lite_manager.py" >/dev/null 2>&1; then
    echo "运行状态: ✅ 运行中 (PID: $(pgrep -f lite_manager.py | head -1))"
else
    echo "运行状态: ⛔ 未运行"
fi
echo "--- 最近日志(末尾 30 行) ---"
tail -n 30 /opt/proxy_lite/proxy_lite.log 2>/dev/null || echo "(暂无日志，请先执行 start.sh)"
EOF

# logs.sh: 实时日志
cat > /opt/proxy_lite/logs.sh << 'EOF'
#!/usr/bin/env bash
exec tail -f /opt/proxy_lite/proxy_lite.log
EOF

# ctrl.sh: 独立按键控制菜单
cat > /opt/proxy_lite/ctrl.sh << 'EOF'
#!/usr/bin/env bash
# ==========================================
#   Proxy Controller 独立按键控制面板
#   按数字键选择操作，无需记命令
# ==========================================
while true; do
    clear
    echo "=========================================="
    echo "     Proxy Controller 控制面板"
    echo "=========================================="
    if pgrep -f "lite_manager.py" >/dev/null 2>&1; then
        echo "     运行状态: ✅ 运行中 (PID: $(pgrep -f lite_manager.py | head -1))"
    else
    ...
    echo "     3) 查看状态 (status)"
    echo "     4) 实时日志 (logs)"
    echo "     5) 一键卸载 (uninstall)"
    echo "     0) 退出"
    echo "------------------------------------------"
    read -rp "     请输入数字按键: " key
    case "$key" in
        1) /opt/proxy_lite/start.sh ;;
        2) /opt/proxy_lite/stop.sh ;;
        3) /opt/proxy_lite/status.sh ;;
        4) /opt/proxy_lite/logs.sh ;;
        5) /opt/proxy_lite/uninstall.sh ;;
        0) echo "    已退出"; exit 0 ;;
        *) echo "    无效按键: $key"; sleep 1 ;;
    esac
    read -rp "    按回车返回菜单..." _discard
done
EOF

# uninstall.sh: 一键卸载
cat > /opt/proxy_lite/uninstall.sh << 'EOF'
#!/usr/bin/env bash
# ==========================================================
#  Proxy Controller 一键卸载脚本
#  停止进程 -> 清理文件/配置 -> 提示可移除的依赖包
# ==========================================================
echo "=== Proxy Controller 卸载 ==="
echo "[1/3] 停止运行中的进程..."
pkill -f "lite_manager.py" 2>/dev/null && echo "  [✓] 已停止 lite_manager" || echo "  [-] lite_manager 未在运行"
pkill -f "openvpn.*tun_main|openvpn.*tun_backup" 2>/dev/null && echo "  [✓] 已停止 openvpn 隧道" || true
pkill -f "proxy_server.py" 2>/dev/null && echo "  [✓] 已停止 proxy_server" || true

echo "[2/3] 删除程序文件与配置..."
rm -rf /opt/proxy_lite && echo "  [✓] 已删除 /opt/proxy_lite"
rm -f /etc/sysctl.d/99-proxy-lite.conf && echo "  [✓] 已删除 sysctl 配置"
sysctl -w net.ipv4.conf.all.rp_filter=1 >/dev/null 2>&1 || true
sysctl -w net.ipv4.conf.default.rp_filter=1 >/dev/null 2>&1 || true

echo "[3/3] 完成！"
echo ""
echo "如需移除依赖包，请按系统手动执行："
echo "  Alpine:       apk del openvpn python3 curl iproute2 iptables busybox-extras"
echo "  Debian/Ubuntu:apt-get remove -y openvpn python3 curl iproute2 iptables psmisc"
echo "  RHEL/CentOS:  dnf remove -y openvpn python3 curl iproute iptables procps-ng"
echo "  Arch:         pacman -R openvpn python3 curl iproute2 iptables procps-ng"
echo "=== 卸载完成 ==="
EOF

chmod +x /opt/proxy_lite/start.sh /opt/proxy_lite/stop.sh /opt/proxy_lite/status.sh /opt/proxy_lite/logs.sh /opt/proxy_lite/ctrl.sh /opt/proxy_lite/uninstall.sh

# ---------- 6. 启动 ----------
echo "[✓] 启动引擎(独立触发)..."
/opt/proxy_lite/start.sh

echo "=========================================================="
echo "  [+] 部署完成！(多系统兼容 · 独立触发版)  "
echo "  控制面板(按键菜单): /opt/proxy_lite/ctrl.sh  "
echo "  启动: /opt/proxy_lite/start.sh  "
echo "  停止: /opt/proxy_lite/stop.sh  "
echo "  状态: /opt/proxy_lite/status.sh  "
echo "  日志: /opt/proxy_lite/logs.sh  "
echo "  卸载: /opt/proxy_lite/uninstall.sh  "
echo "=========================================================="
echo "  输入 ctrl.sh 即可用数字按键管理，无需记命令。"
`;
            ...
            <div class="lg:col-span-3 bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20 flex flex-col justify-center relative overflow-hidden">
                ...
                <div class="flex flex-wrap items-center bg-slate-950/50 border border-slate-800/80 rounded-xl p-5 relative z-10 gap-y-4">
                    <div class="flex items-center gap-3 mr-3 border-r border-slate-700/50 pr-4">
                        ...
                        <select id="slot-cfg-0" class="bg-slate-900 border border-slate-700 rounded-lg py-2 w-24 text-white font-bold text-lg uppercase text-center focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all shadow-inner cursor-pointer">
    ...
    <script>
        ...
        async function fetchCountries() {
            try {
                ...
                container.innerHTML = list.map(c => \`<span onclick="selectCountry('\${c}')" class="bg-slate-800/80 hover:bg-indigo-500/30 hover:text-white text-slate-300 transition-colors border border-slate-700/50 px-2.5 py-1 rounded-md text-xs font-mono font-bold shadow-sm cursor-pointer" title="点击切换到 \${c}">\${c}</span>\`).join('');
                const sel = document.getElementById('slot-cfg-0');
                const cur = sel.value || 'JP';
        ...
        function selectCountry(code) {
            document.getElementById('slot-cfg-0').value = code;
            document.getElementById('slot-cfg-0').focus();
            document.getElementById('slot-cfg-0').dispatchEvent(new Event('change'));
        }
