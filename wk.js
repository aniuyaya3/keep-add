...
if (url.pathname === "/agent") {
  const agentScript = #!/bin/sh  echo "==========================================================" echo "     Proxy Controller (Alpine/OpenRC)" echo "=========================================================="  mkdir -p /opt/proxy_lite/configs cd /opt/proxy_lite || exit 1  echo "[1/5] 安装依赖..."  apk update  apk add --no-cache \     python3 \     curl \     openvpn \     iproute2 \     iptables \     openrc  echo "[2/5] 拉取核心组件..."  curl -fsSL -o lite_manager.py ${domain}/scripts/lite_manager.py curl -fsSL -o proxy_server.py ${domain}/scripts/proxy_server.py  chmod 755 lite_manager.py chmod 755 proxy_server.py  echo "[3/5] 创建 OpenRC 服务..."  cat >/etc/init.d/proxy-lite <<'EOF' #!/sbin/openrc-run  name="proxy-lite" description="Proxy Controller"  command="/usr/bin/python3" command_args="-u /opt/proxy_lite/lite_manager.py"  pidfile="/run/proxy-lite.pid"  command_background="yes"  output_log="/opt/proxy_lite/stdout.log" error_log="/opt/proxy_lite/stderr.log"  depend() {     need net     after firewall }  start_pre() {     mkdir -p /run }  start_post() {     eend 0 }  stop_post() {     rm -f ${pidfile} } EOF  chmod +x /etc/init.d/proxy-lite  echo "[4/5] 注册开机启动..."  rc-update add proxy-lite default  echo "[5/5] 启动服务..."  rc-service proxy-lite stop >/dev/null 2>&1 || true pkill -f lite_manager.py >/dev/null 2>&1 || true  rc-service proxy-lite start  sleep 2  echo echo "==========================================================" echo "安装完成" echo "=========================================================="  if rc-service proxy-lite status >/dev/null 2>&1; then     echo "[+] proxy-lite 已启动" else     echo "[!] 服务状态未知，请检查日志" fi  echo echo "日志:" echo "tail -f /opt/proxy_lite/stdout.log" echo ;
